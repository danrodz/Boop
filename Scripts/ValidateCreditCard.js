...
function main(state) {
  const number = state.text.trim().replace(/[\s-]/g, '');
  ...
  if (isValid) {
    state.text = `✓ Valid credit card number (Luhn check passed)

Type: ${cardType}
Length: ${number.length} digits
Masked: ${'*'.repeat(number.length - 4)}${number.slice(-4)}`;
  } else {
    state.text = `✗ Invalid credit card number (Luhn check failed)

Detected type: ${cardType}`;
  }
}
